...
final class DefaultCommitStorageStrategy: CommitStorageStrategy {
    ...
    init(
        ...
        blobStrategy: FirestoreBlobStrategy,
        branchStrategy: FirestoreBranchStrategy
    ) {
        self.db             = db
    ...
    func saveCommit(_ commit: Commit, localRootURL: URL, branchID: String) async throws -> ProjectVersion {
        ...
        for fileSnapshot in commit.fileSnapshot where fileSnapshot.path.hasSuffix("/") == false {
            let fileURL = localRootURL.appendingPathComponent(fileSnapshot.path)
            let storagePath = "projects/\(commit.projectID)/blobs/\(fileSnapshot.hash)"
