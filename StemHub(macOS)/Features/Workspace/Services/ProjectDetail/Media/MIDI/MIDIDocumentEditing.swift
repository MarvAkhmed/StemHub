...
private enum MIDIDocumentIO {
    nonisolated static func loadDocumentSynchronously(from session: ProjectMIDISession) throws -> MIDIDocument {
        ...
        return try session.fileURL.withSecurityScopedAccess {
    ...
    nonisolated static func saveDocumentSynchronously(_ document: MIDIDocument) throws {
        try document.fileURL.deletingLastPathComponent().withSecurityScopedAccess {
    ...
    nonisolated static func requireNoErr(_ status: OSStatus,
                                         error errorBuilder: (OSStatus) -> MIDIEditorError) throws {
