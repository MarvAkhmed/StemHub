//
//  CoreMIDIControllerMonitoringSessionImpl.swift
//  StemHub_macOS
//
...
final class CoreMIDIControllerMonitoringSessionImpl: MIDIControllerMonitoringSession {
...
private extension CoreMIDIControllerMonitoringSessionImpl {
    ...
    func handleEventList(_ eventList: UnsafePointer<MIDIEventList>) {
        ...
        MIDIEventListForEachEvent(eventList, { context, timeStamp, message in
            ...
            if let event = session.makeLiveEvent(from: message, timeStamp: timeStamp) {
                session.emit(event)
            }
        }, context)
    }
    ...
    private func makeLiveEvent(from message: MIDIUniversalMessage,
                               timeStamp: MIDITimeStamp) -> MIDILiveEvent? {
        switch message.type {
        case .channelVoice1:
    ...
    private func makeLiveEventFromChannelVoice1(_ message: MIDIUniversalMessage,
                                                timeStamp: MIDITimeStamp) -> MIDILiveEvent? {
    ...
    private func makeLiveEventFromChannelVoice2(_ message: MIDIUniversalMessage,
                                                timeStamp: MIDITimeStamp) -> MIDILiveEvent? {
