...
final class SyncOrchestrator {
    ...
    func pullProject(
        ...
        let versionsToApply = try await versionLineage(
            headVersionID: latestVersionID,
            baseVersionID: updatedState.lastPulledVersionID
        )

        for version in versionsToApply {
            try await apply(version: version, to: localRootURL)
            updatedState.lastPulledVersionID = version.id
        }
...
private extension SyncOrchestrator {
    func versionLineage(
        ...
        while let versionID = nextVersionID, versionID != baseVersionID {
    ...
    func apply(version: ProjectVersion, to localRootURL: URL) async throws {
        ...
        for diff in version.diff.files {
