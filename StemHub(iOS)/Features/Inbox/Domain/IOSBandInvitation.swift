...
enum IOSBandInvitationStatus: String, Codable, CaseIterable, Sendable {
    case pending
    case accepted
    case declined
...
struct IOSBandInvitation: Identifiable, Codable, Hashable, Sendable {
    let id: String
    let bandID: String
    let bandName: String
    ...
    var status: IOSBandInvitationStatus
    let createdAt: Date
    ...
    init(
        ...
        status: IOSBandInvitationStatus,
        createdAt: Date,
        respondedAt: Date?
    ) {
        self.id = id
    ...
    init(from decoder: Decoder) throws {
        ...
        status = try container.decodeIfPresent(IOSBandInvitationStatus.self, forKey: .status) ?? .pending
