...
final class SyncOrchestrator {
    ...
    private let diffStrategy: DiffCalculationStrategy
    ...
    init(
        ...
        diffStrategy: DiffCalculationStrategy = StandardDiffEngine(),
        commitStorage: CommitStorageStrategy = FirestoreCommitStorage(),
        remoteFetch: RemoteFetchStrategy = FirestoreRemoteFetch(),
        fileUploadStrategy: FileUploadStrategy = FirebaseStorageUpload()
    ) {
